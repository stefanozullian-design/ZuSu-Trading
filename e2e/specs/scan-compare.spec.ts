...
test.describe('comparing scans', () => {
  ...
  test('lays two filters side by side and counts the agreement', async ({ page }) => {
    ...
    await expect(panel.getByRole('button', { name: `E2E Loose RSI ${RUN}` })).toBeVisible();

    await page.getByLabel('Value').first().fill('99');
    await page.getByLabel('Save this filter as').fill(`E2E Looser RSI ${RUN}`);
    await page.getByRole('button', { name: /^save$/i }).click();
    await expect(panel.getByRole('button', { name: `E2E Looser RSI ${RUN}` })).toBeVisible();

    await panel.getByRole('button', { name: `E2E Loose RSI ${RUN}` }).click();
    await panel.getByRole('button', { name: `E2E Looser RSI ${RUN}` }).click();
  ...
  test('prints the caveats beside the count, not behind a tooltip', async ({ page }) => {
    ...
    await panel.getByRole('button', { name: `E2E Caveat Check ${RUN}` }).click();
